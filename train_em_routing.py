...
import dataset
from model_em_routing import EM_Routing_Net
    ...
    with tf.Graph().as_default():
        ...
        for step in range(cfg.epoch * num_batch_per_epoch_train):
            ...
            try:
                ...
                session.run(train_ops.train_op, feed_dict={model.learning_rate: learning_rate, model.thres_op: thres})
            ...
            if step > 0 and step % num_batch_per_epoch_train == 0:
                ...
                thres += (thres_max-thres_min)/(cfg.epoch*cfg.thres_schedule)
                if thres > thres_max:
                    thres = thres_max
