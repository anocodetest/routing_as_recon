...
import dataset
from model_wrn import Wide_Resnet
    ...
    with tf.Graph().as_default():
        ...
        for step in range(cfg.epoch * num_batch_per_epoch_train):
            ...
            try:
                ...
                print('%d iteration finished in ' % step + '%f second,' % (time.time() - tic))
            ...
            if step > 0 and step % num_batch_per_epoch_train == 0:
                ...
                if step >= cfg.epoch*num_batch_per_epoch_train*cfg.decay_prop_0 and cfg.decay_prop_0 > 0.0:
                    learning_rate = cfg.init_learning_rate*cfg.decay_factor

                if step >= cfg.epoch*num_batch_per_epoch_train*cfg.decay_prop_1 and cfg.decay_prop_1 > 0.0:
                    learning_rate = cfg.init_learning_rate*cfg.decay_factor*cfg.decay_factor

                if step >= cfg.epoch*num_batch_per_epoch_train*cfg.decay_prop_2 and cfg.decay_prop_2 > 0.0:
                    learning_rate = cfg.init_learning_rate*cfg.decay_factor*cfg.decay_factor*cfg.decay_factor
